...
from robot_simulator.physics import (
    ...
    Body, RobotTree,
    RevoluteJoint, FixedJoint, FreeJoint, Axis,
...
def build_quadruped() -> tuple[RobotTree, ContactModel, AABBSelfCollision]:
    ...
    contact_model = ContactModel(ContactParams(
        k_normal = 1_000.0,
        b_normal = 300.0,
        mu       = 0.7,
    ))
    ...
    self_collision.add_body(BodyAABB(
    ...
    for leg_name, (sx, sy) in leg_signs.items():
        ...
        hip = Body(
            ...
            index   = 0,
            joint   = RevoluteJoint(
                f"{leg_name}_hip_joint", Axis.X,
                q_min=HIP_Q_MIN, q_max=HIP_Q_MAX,
        ...
        thigh = Body(
            ...
            index   = 0,
            joint   = RevoluteJoint(
                f"{leg_name}_thigh_joint", Axis.Y,
                q_min=THIGH_Q_MIN, q_max=THIGH_Q_MAX,
                k_limit=K_LIMIT, b_limit=B_LIMIT,
            ),
        ...
        calf = Body(
            ...
            index   = 0,
            joint   = RevoluteJoint(
                f"{leg_name}_calf_joint", Axis.Y,
                q_min=CALF_Q_MIN, q_max=CALF_Q_MAX,
                k_limit=K_LIMIT, b_limit=B_LIMIT,
            ),
            inertia = calf_inertia,
            X_tree  = SpatialTransform(np.eye(3), np.array([0., 0., -THIGH_LENGTH])),
            parent  = thigh_idx,
        )
        ...
        foot = Body(
            ...
            X_tree  = SpatialTransform(np.eye(3), np.array([0., 0., -CALF_LENGTH])),
            parent  = calf_idx,
        )
        foot_idx = tree.add_body(foot)

        # Contact point at foot body origin (= geometric foot tip)
        contact_model.add_contact_point(ContactPoint(
            body_index = foot_idx,
            position   = np.zeros(3),
            name       = f"{leg_name}_foot",
        ))
    ...
    for i in range(n_steps):
        ...
        if i % 200 == 0:
            ...
            print(f"  t={times[i]:.2f}s  torso_z={z:.3f}m  "
                  f"contacts={[c[0] for c in active]}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save", metavar="PATH", default=None,
                        help="Save animation to .gif or .mp4 (requires pillow/ffmpeg)")
