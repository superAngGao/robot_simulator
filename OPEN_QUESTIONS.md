...
**Q28 — GPU ADMM 多体同时接触发散** (2026-03-30)

两球同时着地+互碰场景，GPU ADMM 在 ~1000 步后 NaN。PGS 同场景不发散。

- 复现：两球从 z=0.5 水平相向运动，着地后 body-body + ground contact 同时激活
- 表现：角速度先增长到 ~7000 rad/s，然后 qdot → NaN
- 可能原因：
  - ADMM compliance 模型允许更深穿透 (~15mm)，多接触重叠时 Delassus 条件数恶化
  - body-body + ground 接触同时激活时 R_diag 计算可能不稳定
  - Cholesky reg=1e-4 对多接触场景可能不够
- PGS 不发散的原因：PGS 的 ERP + split impulse 更保守地限制穿透
- **优先级**：P1（影响多体 RL 场景）
- **待调查**：对比 ADMM 和 PGS 在发散前的 lambda、W、R_diag 数值
