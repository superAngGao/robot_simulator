...
因此 Q51 对 Q50 Step 4 的阻塞已关闭；剩余问题转为后续 RL obs schema / telemetry
parity / 传感器坐标系扩展时再处理。
...
- RL obs 复用、训练期更完整传感器可视化仍需后续 schema
...
- 待 RL 观测接口进入实现前，专门回到 quaternion / contact mask / normalization 讨论

**触发条件**：需要把 GPU 训练信号接入 RL obs 或更完整的 Rerun scalar/tensor timeline 时。
**优先级**：P2（Q50 Step 4 阻塞已解除；后续 RL obs 前需再次收敛）。
