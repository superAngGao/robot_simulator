...
**Q21 — PGS Baumgarte 在持续多步接触中发散（2026-03-24 发现）**

**现象**：两球撞墙场景中，PGS + Baumgarte ERP 在球持续接触墙面时速度发散
（vx 从 3 m/s 飙升到 3480 m/s）。原因是 ERP 位置修正通过 force→accel→vel→pos
链传递，在持续接触中形成正反馈振荡。

**ADMM 不受影响**：同一场景使用 ADMM 求解器完全稳定（max |vx|=4.6），因为
ADMM 的隐式耦合 `(M+ρJᵀJ)v=...` 天然阻止振荡。

**主流项目的解法**：
- Bullet: split impulse（位置修正和速度约束分两个 pass，解耦正反馈）
- MuJoCo: 隐式积分（约束和动力学耦合求解，等价于 ADMM）
- PhysX: Temporal GS + bias clamp（PGS 变体 + 位置修正上限）
- ODE: 高 CFM 柔化约束（允许穿透换取稳定）

**待解决方案**（按优先级）：
1. ~~Simulator 默认求解器改为 ADMM~~ — 已验证可行，需要性能优化后切换
2. PGS 加 split impulse — Bullet 方案，工程量中等
3. PGS 加 velocity clamping — 最简单但最 hacky

**当前状态**：Simulator 支持 `solver=ADMMContactSolver(...)` 传入，用户可手动选择。
默认仍是 PGS（向后兼容）。应在 ADMM GPU kernel 完成后将默认切换为 ADMM。
