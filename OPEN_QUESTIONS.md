...
2026-04-30 L2 acceleration review request：

- 本地未安装 / 未声明 `embree`、`pyembree`、`embreex` 或 `open3d`。
- 建议 L2 第一版走 in-repo simple BVH，不引入 Open3D 重依赖。
- Embree 保留为后续 optional adapter，不作为当前硬依赖。
- BVH/acceleration payload 建议归属 `OpticalSceneSnapshot` / scene-cache，
  不放 registry，也不让 executor 静默现建。
- infinite plane 保持 analytical pass，不放入 BVH。
- `CpuBvhOpticalExecutor` 建议要求 snapshot 已有 `cpu_bvh` acceleration，
  缺失时 raise，避免模糊 scene/executor 边界。
- 自研算法推进规则：L2 BVH 等 self-authored algorithm 在实现前必须先写清楚
  inputs/outputs、data layout、核心算法、退化行为、复杂度、测试策略和 non-goals，
  先 review 再编码。

详见：
`collab/q54-optical-l2-acceleration__review-request__codex__v1.md`。

2026-05-01 L2 BVH algorithm plan：

- 已明确第一版 BVH 路线：in-repo CPU BVH、world-space packed triangles、
  longest-axis median split、`leaf_size=4`、iterative stack traversal。
- scene/cache opt-in build acceleration，executor consume-only。
- infinite plane 不进 BVH，作为 analytical side pass 和 BVH triangle hit 合并。
- 显式 lexicographic source-order-key tie-break 对齐 reference executor 的扫描顺序：
  `(instance_index, primitive_index_within_instance)`；plane 和 triangle 都按全局
  `snapshot.instances` 顺序参与 tie-break，不按 geometry kind 分段。
- missing acceleration 建议使用专用 `MissingAccelerationError`，不 fallback。
- `build_eps=1e-12` 用于几何退化；`dir_eps=1e-12` 用于 slab parallel；
  `t_eps=1e-9` 用于 hit-distance tie-break。
- 第一版目标是 correctness / schema parity，不做 SAH、refit、Embree、GPU、
  direct-light 或 shadow rays。

详见：
`collab/q54-optical-l2-bvh-algorithm-plan__codex__v1.md`。
Claude review 归档：
`collab/q54-optical-l2-bvh-algorithm-plan__review__claude__v1.md`。
Claude follow-up 确认：
`collab/q54-optical-l2-bvh-algorithm-plan__review-followup__claude__v1.md`。

2026-05-01 L2 BVH implementation status：

- 已实现 `OpticalSceneAcceleration`、`CpuBvhNode` 和 scene/cache opt-in
  `acceleration="cpu_bvh"`。
- `OpticalSceneSnapshot.acceleration` 承载 scene/cache-owned acceleration payload；
  默认仍为 `None`。
- 已实现 `CpuBvhOpticalExecutor` 和 `MissingAccelerationError`。
- BVH executor 保持 `OpticalComputeResult` schema，与 reference executor capabilities
  一致。
- triangle mesh 走 world-space packed BVH；infinite plane 继续 analytical side pass。
- tie-break 使用全局 `(instance_index, primitive_index_within_instance)`，plane 和
  triangle 都按 `snapshot.instances` 顺序参与。
- 新增 `tests/unit/optics/test_bvh_executor.py` 覆盖 packing、internal traversal、
  plane side pass、tie-break、near-equal non-coplanar hit、roles filtering、missing
  acceleration 和 camera postprocess。
- 验证：`PYTHONPATH=. pytest tests/unit/optics tests/unit/sensing -q` -> `66 passed`。
...
**Q54-OPEN — P6 runner integration must preserve `frame_source` timing metadata**

P5 added a test-only physics video workflow smoke:
`GpuEngine.step()` → physics frame-context provider → `FrameWorkflowRunner` →
video delivery → `FrameTimingRecorder`. This validates the provider/runtime/
delivery ordering, but it intentionally bypasses `run_scenario(...)` and
constructs lab frame defaults directly inside the test.

Current status:
- `FRAME_TIMING_FIELDNAMES` now includes `frame_source`.
- Formal runner paths populate `lab_frame_defaults["frame_source"]` via
  `frame_defaults_for_config(...)`.
- The P5 test-only smoke does not validate the full runner metadata path.
- Device/width/height/scene_preset defaults are likewise not the purpose of
  the P5 smoke.

Open decision:
- When P6 enables the smallest `FrameSourceKind.PHYSICS_RUNTIME` runner path,
  confirm the frame timing CSV is built from `frame_defaults_for_config(...)`
  rather than ad-hoc defaults.
- Add an end-to-end smoke assertion that `frame_source ==
  "physics_runtime"` appears in `frame_timing.csv`.

Suggested trigger:
- Resolve this as part of P6 before relaxing the
  `FrameSourceKind.PHYSICS_RUNTIME` guard in `run_scenario(...)`.
