...
Current optical lab work extends that foundation with GPU device-scene executors, BVH traversal, direct lighting and shadow rays, CUDA LBVH rebuilds, GPU pinhole ray generation, and a source-driven render foundation. The production-facing dynamic path starts from physics-published frames. Static asset sources are only for non-simulated assets: `static_asset_source.py` constructs optical registries and base render sources, while Go2/Menagerie code only supplies the concrete Menagerie model, camera presets, and preview/reporting glue. Both static assets and physics frames feed the same generic render session, workspace, frame preparation, and video-loop helpers under `tools/optical_pipeline_lab/`.
