...
Version: v1
Date: 2026-05-06
Status: v1-implemented-with-gpu-raygen-followup
...
camera ray generation + broad host staging dominate the no-write frame.
...
camera_orbit steady-state per-frame timing should reveal render/stage cost more directly.
```
...
## Optimization Area 2: Host Staging / Readback

Problem:

```text
stage_host_mean ~= 29-32 ms at 960x640
```
...
This stages the full result, including channels that a render-only benchmark or
...
2. Add rgb-only staging mode:
   stage only rgb and scalar diagnostics needed for overflow reporting.

3. Keep full staging mode for parity/debug.
```

Potential CLI:

```text
--video-stage full|rgb|diagnostics|none
```
...
```text
full:
  current behavior; stage all channels

rgb:
  stage only rgb plus small diagnostics

diagnostics:
  stage only scalar overflow/max-stack diagnostics if possible

none:
  do not stage host data; only wait result.ready_event.
...
```text
Can we stage individual Warp arrays cleanly with existing helper APIs, or should
...
```text
Add a small selected-channel staging helper rather than duplicating staging
...
This is meaningful, but not yet isolated from ray generation and staging.
...
```text
Do not start here until ray-cache and selected staging modes make render-only
...
possibly optics staging helper
```
...
2. Add --video-stage full|rgb|none.
3. Add selected-channel staging helper if needed.
...
```text
baseline:
  uncached rays + full stage + no write

ray-cache:
  precomputed rays + full stage + no write

render-only:
  precomputed rays + no stage + no write

rgb-output:
  precomputed rays + rgb stage + rgb PNG write
```
...
1. Is the V1 focus on ray cache + selected staging the right next step, or
...
3. Should selected-channel staging be implemented as a reusable optics helper
   now, or kept local to the example until the session/runtime API is clearer?

4. Is `--video-stage full|rgb|none` the right surface, or do we need a more
   explicit channel list such as `--video-stage-channels rgb,bvh_stack...`?
...
camera rays and staging must be addressed first.
...
```text
V1 scope:
  Do ray cache + selected staging + rgb-only write first.
...
Selected staging:
  Put selected-channel staging in an optics helper, not duplicated in the
  example, because Warp ownership/stream synchronization details are easy to
  get wrong.

Video stage CLI:
  Use --video-stage full|rgb|none for V1.
...
Skipped phases:
  ...
  measured stage took zero time; NaN means the stage was not part of that
  benchmark mode.
```
...
1. Update frame_timing.csv schema:
   add geometry_mode, ray_cache_mode, stage_mode, write_mode.
   emit NaN for snapshot/refit/rebuild/stage/image/write when not applicable.
...
3. Add a narrow selected-channel staging helper in optics:
   stage_optical_channels(result, channels: Sequence[str]) -> dict[str, np.ndarray]

4. Add --video-stage full|rgb|none.
   full uses current stage_optical_compute_result_to_host.
   rgb stages rgb plus minimal diagnostics needed for overflow if practical.
   none skips host staging and reports diagnostics as NaN/unavailable.
...
6. Re-run 960x640 matrix:
   ...
   precomputed rays + full stage + no write
   precomputed rays + no stage + no write
   precomputed rays + rgb stage + rgb write
```
...
frame_timing.csv mode columns:
  geometry_mode, ray_cache_mode, stage_mode, write_mode
...
Selected RGB staging cuts host staging from ~32-35 ms to ~6 ms.
