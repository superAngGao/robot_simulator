...
from physics.backends.warp.spatial_warp import (
    mat66f,
    vec6f,
    rodrigues_wp,
    quat_to_rot_wp,
...
class TestRodrigues:
    def test_identity(self):
        ...
        wp.launch(_test_rodrigues, dim=1, inputs=[
...
class TestQuatToRot:
    def test_identity_quat(self):
        out = wp.zeros(1, dtype=wp.mat33)
        wp.launch(_test_quat_to_rot, dim=1, inputs=[
...
class TestTransformVelocity:
    def test_vs_numpy(self):
        ...
        wp.launch(_test_transform_velocity, dim=1, inputs=[
...
class TestTransformForce:
    def test_vs_numpy(self):
        ...
        out = wp.zeros(1, dtype=vec6f)
        wp.launch(_test_transform_force, dim=1, inputs=[
            wp.array(R_f32, dtype=wp.mat33),
            wp.array(r_f32, dtype=wp.vec3),
...
class TestCompose:
    def test_vs_numpy(self):
        ...
        wp.launch(_test_compose, dim=1, inputs=[
...
class TestInverse:
    def test_vs_numpy(self):
        ...
        out_R = wp.zeros(1, dtype=wp.mat33)
        out_r = wp.zeros(1, dtype=wp.vec3)
        wp.launch(_test_inverse, dim=1, inputs=[
            wp.array(R_f32, dtype=wp.mat33),
            wp.array(r_f32, dtype=wp.vec3),
            out_R, out_r,
        ])
...
class TestSpatialCross:
    def test_velocity_cross_vs_numpy(self):
        ...
        out = wp.zeros(1, dtype=vec6f)
        wp.launch(_test_spatial_cross_vel, dim=1, inputs=[
            wp.array(v_f32, dtype=vec6f),
    ...
    def test_force_cross_vs_numpy(self):
        ...
        f_f32 = f.astype(np.float32).reshape(1, 6)
        out = wp.zeros(1, dtype=vec6f)
        wp.launch(_test_spatial_cross_force, dim=1, inputs=[
            wp.array(v_f32, dtype=vec6f),
            wp.array(f_f32, dtype=vec6f),
            out,
        ])
...
class TestTransformMatrix:
    def test_vs_numpy(self):
        ...
        wp.launch(_test_transform_matrix, dim=1, inputs=[
            wp.array(R_f32, dtype=wp.mat33),
            wp.array(r_f32, dtype=wp.vec3),
            out,
        ])
    ...
    def test_matrix_velocity_consistency(self):
        ...
        wp.launch(_test_mat66_mul_vec6, dim=1, inputs=[
            out_M,
            wp.array(v_f32, dtype=vec6f),
            out_v,
        ])
