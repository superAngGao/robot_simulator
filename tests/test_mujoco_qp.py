...
from physics.joint import FreeJoint, RevoluteJoint, Axis
...
def _make_ball_tree(mass=MASS):
    ...
    tree.add_body(Body("ball", 0, FreeJoint("root"),
        SpatialInertia(mass, np.diag([I_DIAG]*3), np.zeros(3)),
        SpatialTransform.identity(), parent=-1))
    tree.finalize()
    return tree
...
def _run_ball_drop(tree, solver, n_steps=500, z0=0.3):
    ...
    for i in range(n_steps):
        ...
        if m is not None and m.depth > 0:
            cc.append(ContactConstraint(body_i=0, body_j=-1,
                point=m.points[0], normal=m.normal.copy(),
                tangent1=np.zeros(3), tangent2=np.zeros(3),
                depth=m.depth, mu=0.5, condim=3))
...
class TestContactJacobian:
    ...
    def test_single_revolute_analytical(self):
        tree = RobotTreeNumpy(gravity=G)
        tree.add_body(Body("link", 0, RevoluteJoint("j0", axis=Axis.Z),
            SpatialInertia(1.0, np.diag([0.01]*3), np.zeros(3)),
            SpatialTransform(np.eye(3), np.zeros(3)), parent=-1))
        tree.finalize()
        q = np.array([0.0])
    ...
    def test_double_pendulum_analytical(self):
        tree = RobotTreeNumpy(gravity=G)
        tree.add_body(Body("l1", 0, RevoluteJoint("j0", axis=Axis.Z),
            SpatialInertia(1.0, np.diag([0.01]*3), np.zeros(3)),
            SpatialTransform(np.eye(3), np.zeros(3)), parent=-1))
        tree.add_body(Body("l2", 1, RevoluteJoint("j1", axis=Axis.Z),
            SpatialInertia(1.0, np.diag([0.01]*3), np.zeros(3)),
            SpatialTransform(np.eye(3), np.array([1, 0, 0])), parent=0))
        tree.finalize()
        q = np.array([0.0, 0.0])
        X = tree.forward_kinematics(q)
...
class TestSingleStep:
    ...
    def test_contact_force_positive_upward(self):
        ...
        a_u = tree.aba(q, qdot, np.zeros(tree.nv))
        X = tree.forward_kinematics(q)
        cc = [ContactConstraint(body_i=0, body_j=-1,
            point=np.array([0, 0, 0.0]), normal=np.array([0, 0, 1.0]),
            tangent1=np.zeros(3), tangent2=np.zeros(3),
            depth=0.001, mu=0.5, condim=3)]
...
class TestMuJoCoTrajectory:
    ...
    def test_settling_time_match(self, trajectories):
        ...
        assert abs(our_settle - mj_settle) <= 1, (
            f"Settle mismatch: ours={our_settle}, mujoco={mj_settle}"
        )
