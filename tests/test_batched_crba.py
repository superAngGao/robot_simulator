...
class TestBatchedCRBAvsABA:
    def test_zero_state(self, model, static):
        ...
        for i in range(N):
            np.testing.assert_allclose(qddot_gpu_np[i], qddot_aba, atol=ATOL,
                                       err_msg=f"Batched CRBA env {i} != ABA")
    ...
    def test_nonzero_tau(self, model, static):
        ...
        np.testing.assert_allclose(qddot_gpu[0].cpu().numpy(), qddot_aba,
                                   atol=ATOL, rtol=1e-4,
