...
from physics.joint import FreeJoint, RevoluteJoint, FixedJoint
...
class TestCRBAvsABA:
    ...
    def test_zero_state(self):
        ...
        np.testing.assert_allclose(qddot_crba, qddot_aba, atol=1e-10,
                                   err_msg="CRBA != ABA at zero state")
    ...
    def test_nonzero_tau(self):
        ...
        np.testing.assert_allclose(qddot_crba, qddot_aba, atol=1e-10,
                                   err_msg="CRBA != ABA with nonzero tau")
    ...
    def test_nonzero_state(self):
        ...
        np.testing.assert_allclose(qddot_crba, qddot_aba, atol=1e-10,
                                   err_msg="CRBA != ABA at nonzero state")
    ...
    def test_quadruped_zero_state(self):
        ...
        np.testing.assert_allclose(qddot_crba, qddot_aba, atol=1e-10,
                                   err_msg="CRBA != ABA for quadruped at zero state")
    ...
    def test_quadruped_random(self):
        ...
        np.testing.assert_allclose(qddot_crba, qddot_aba, atol=1e-9,
                                   err_msg="CRBA != ABA for quadruped random state")
    ...
    def test_with_external_forces(self):
        ...
        np.testing.assert_allclose(qddot_crba, qddot_aba, atol=1e-10,
                                   err_msg="CRBA != ABA with external forces")
...
class TestRoundtrip:
    ...
    def test_roundtrip(self):
        ...
        np.testing.assert_allclose(tau_reconstructed, tau, atol=1e-10,
                                   err_msg="H @ qddot + C != tau")
...
class TestPinocchioComparison:
    ...
    def test_mass_matrix_vs_pinocchio(self):
        ...
        model.appendBodyToJoint(j1_id, pin.Inertia(0.5, np.array([0, 0, -0.1]), np.diag([0.001, 0.001, 0.001])), pin.SE3.Identity())
        # Link2 (revolute Y)
        j2_placement = pin.SE3(np.eye(3), np.array([0.0, 0.0, -0.2]))
        j2_id = model.addJoint(j1_id, pin.JointModelRY(), j2_placement, "j2")
        model.appendBodyToJoint(j2_id, pin.Inertia(0.3, np.array([0, 0, -0.1]), np.diag([0.001, 0.001, 0.001])), pin.SE3.Identity())
        ...
        np.testing.assert_allclose(H_ours, H_pin, atol=1e-8,
                                   err_msg="CRBA mass matrix differs from Pinocchio")
