...
class TestFKAndVelocities:
    ...
    def test_fk_default_state(self, model, cfg):
        ...
        np.testing.assert_allclose(
            res_wp.X_world.numpy(), res_np.X_world.numpy(), atol=ATOL_SINGLE
        )
        # v_bodies (all zero at default state)
        np.testing.assert_allclose(
            res_wp.v_bodies.numpy(), res_np.v_bodies.numpy(), atol=ATOL_SINGLE
        )
...
class TestSingleStep:
    ...
    def test_zero_action_step(self, model, cfg):
        ...
        np.testing.assert_allclose(
            res_wp.q.numpy(), res_np.q.numpy(), atol=ATOL_SINGLE,
            err_msg="q mismatch after single zero-action step"
        )
        np.testing.assert_allclose(
            res_wp.qdot.numpy(), res_np.qdot.numpy(), atol=ATOL_SINGLE,
...
class TestMultiStep:
    ...
    def test_50_steps_free_fall(self, model_no_contact, cfg):
        ...
        np.testing.assert_allclose(
            res_wp.q.numpy(), res_np.q.numpy(), atol=ATOL_MULTI,
            err_msg="q diverged after 50 free-fall steps"
        )
        np.testing.assert_allclose(
            res_wp.qdot.numpy(), res_np.qdot.numpy(), atol=ATOL_MULTI,
