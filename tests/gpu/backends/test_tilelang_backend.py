"""
Tests for TileLangBatchBackend — numerical validation against NumpyLoopBackend.
"""
...
class TestFKAndVelocities:
    def test_fk_default_state(self, model, cfg):
        ...
        tl_b = get_backend("tilelang", model, cfg, num_envs=2)
        res_np = np_b.reset_all()
        res_tl = tl_b.reset_all()
        np.testing.assert_allclose(res_tl.X_world.numpy(), res_np.X_world.numpy(), atol=ATOL_SINGLE)
        np.testing.assert_allclose(res_tl.v_bodies.numpy(), res_np.v_bodies.numpy(), atol=ATOL_SINGLE)
...
class TestSingleStep:
    def test_zero_action_step(self, model, cfg):
        ...
        res_np = np_b.step_batch(actions)
        res_tl = tl_b.step_batch(actions)
        np.testing.assert_allclose(res_tl.q.numpy(), res_np.q.numpy(), atol=ATOL_SINGLE)
        np.testing.assert_allclose(res_tl.qdot.numpy(), res_np.qdot.numpy(), atol=ATOL_SINGLE)
...
class TestMultiStep:
    def test_50_steps_free_fall(self, model_no_contact, cfg):
        ...
        tl_b = get_backend("tilelang", model_no_contact, cfg, num_envs=1)
        np_b.reset_all()
        tl_b.reset_all()
        ...
        for _ in range(50):
            res_np = np_b.step_batch(actions)
            res_tl = tl_b.step_batch(actions)
        np.testing.assert_allclose(res_tl.q.numpy(), res_np.q.numpy(), atol=ATOL_MULTI)
        np.testing.assert_allclose(res_tl.qdot.numpy(), res_np.qdot.numpy(), atol=ATOL_MULTI)


class TestObservations:
    def test_obs_shapes(self, model, cfg):
        tl_b = get_backend("tilelang", model, cfg, num_envs=4)
        result = tl_b.reset_all()
        obs = tl_b.get_obs_data(result)
        nu = len(model.actuated_joint_names)
...
class TestQuaternionNorm:
    def test_quat_norm_after_steps(self, model_no_contact, cfg):
        ...
        actions = torch.zeros(1, nu)
        for _ in range(100):
            result = tl_b.step_batch(actions)
