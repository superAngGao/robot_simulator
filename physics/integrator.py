    ...
    def _integrate_q(
        ...
        for body in tree.bodies:
            ...
            if isinstance(j, FreeJoint):
                q_new[body.q_idx] = j.integrate_q(
                    q[body.q_idx], qdot[body.v_idx], dt
                )
            else:
...
class SemiImplicitEuler(Integrator):
    ...
    def step(
        ...
        if not np.all(np.isfinite(q_new)):
            raise RuntimeError(
                "SemiImplicitEuler: state diverged (NaN/Inf). "
                "Try reducing dt or contact stiffness k_normal."
            )
...
def simulate(
    ...
    """
    integ: Integrator = (
        SemiImplicitEuler(dt) if integrator == "semi_implicit" else RK4(dt)
    )
    ...
    for i in range(n_steps):
        ...
        if contact_fn is not None:
            from .robot_tree import RobotTree as _RT  # avoid circular at module level
